...
ShipServiceProto.getById = function(name) {
    var d = this.db.one(ShipService.BY_ID_QUERY, name).then(ShipServiceProto._cleanDescription.bind(this));
...
ShipServiceProto._cleanDescription = function(ships) {
    ...
    for(i = 0, l = ships.length; i < l; i += 1) {
        ...
        // Fix the newline horribleness, then strip any inline links, since they're Eve-specific
        desc = ship.description.replace(/\\r\\n/g, '\n\n').replace(/<a[^>]+>/ig, '');
